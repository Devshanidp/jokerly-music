...
export async function GET(req: NextRequest) {
  ...
  // When a bust/refresh param is present, use a random offset so the catalog returns different results
  ...
  const results = await Promise.allSettled(
    langs.map(async (langId) => {
      ...
      const [trackData, artistData] = await Promise.all([
        searchCatalog(lang.query, "track", session!.accessToken, 10, offset),
        searchCatalog(lang.artistQuery, "artist", session!.accessToken, 6, 0),
      ]);
  ...
  results.forEach((r, i) => {
    if (r.status === "rejected") {
      const e = r.reason;
      const status = e instanceof CatalogApiError ? e.status : 0;
