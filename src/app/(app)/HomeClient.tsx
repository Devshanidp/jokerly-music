...
import { Pin, Search, Loader2, Music, Mic2, Play, ListPlus, Languages } from "lucide-react";
...
export default function HomeClient() {
  ...
  // Fetch prefs + pinned in parallel
  useEffect(() => {
    Promise.all([
    ...
    <div className="space-y-8">
      ...
      {langs && langs.length > 0 && (
        <div className="flex items-center justify-between">
          ...
          <Link href="/onboarding" className="flex items-center gap-1.5 text-xs text-white/25 hover:text-white transition-colors shrink-0 ml-3">
            <Languages size={12} /> Edit
          </Link>
        </div>
      )}
