      ...
      <div className="rounded-3xl w-full max-w-sm border border-white/[0.09] shadow-2xl shadow-black/60 overflow-hidden"
        ...
        <div className="p-5 space-y-5">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-[#8b5cf6] hover:bg-[#8b5cf6]/10 transition-colors text-sm font-medium"
          >
            <X size={15} />
            Sign out
          </button>
...
export default function Topbar() {
  ...
  return (
    <>
      {sessionError && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-[#8b5cf6] text-white text-sm px-4 py-2.5 flex items-center justify-between gap-3">
          <span>Your Spotify session expired. Please sign back in.</span>
          <button onClick={() => signOut({ callbackUrl: "/login" })}
            className="shrink-0 bg-white text-[#8b5cf6] font-semibold text-xs px-3 py-1.5 rounded-lg">
            Sign out
          </button>
        </div>
      )}
      <header className={`sticky z-30 shrink-0 ${sessionError ? "top-10" : "top-0"}`}
        style={{ background: "rgba(6,4,14,0.88)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(139,92,246,0.08)" }}>
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Link href="/search"
              className={`p-2 rounded-xl transition-colors ${pathname === "/search" ? "text-[#8b5cf6] bg-[#8b5cf6]/10" : "text-white/40 hover:text-white hover:bg-white/[0.07]"}`}
              title="Search">
              <Search size={17} />
            </Link>
