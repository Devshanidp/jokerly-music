import { MusicAlbum, albumImage } from "@/types/music-catalog";
...
export default function CatalogAlbumCard({ album, onSelect }: Props) {
  ...
  const inner = (
    <>
      ...
      <div>
        <p className="text-white text-sm font-semibold truncate group-hover:text-[var(--accent)] transition-colors">
          {album.name}
        </p>
  ...
  return (
    <a href={album.external_urls.web} target="_blank" rel="noopener noreferrer" className={cls} style={style}>
      {inner}
    </a>
  );
}
